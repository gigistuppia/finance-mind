...
/**
 * ¿El activo opera 24/7? Se deduce de los datos, no de metadatos externos:
 * una serie continua trae ~365 velas por año; una con horario bursátil, ~252.
 *
 * Importa (CLAUDE.md §14.10) porque "el día" y "el hueco de apertura" no
 * significan lo mismo en BTC que en una acción. Sin esto, el informe le
 * explica a un tenedor de cripto que el precio saltó "fuera de horario".
 */
function esMercadoContinuo(candles) {
  if (candles.length < 30) return false;
  const dias = (candles[candles.length - 1].t - candles[0].t) / 86_400_000;
  if (dias <= 0) return false;
  return candles.length / dias > 0.85; // 7/7 ≈ 1.0 · 5/7 ≈ 0.71
}
...
function computeAll(candles) {
  ...
  const snapshot = {
    ...
    // §14.10: define qué significa "el día" para este activo.
    mercado: esMercadoContinuo(candles) ? 'continuo 24/7' : 'con horario de sesión',
...
module.exports = {
  ...
  relativeVolume, drawdown, pivots, clusterLevels, nearestLevels, esMercadoContinuo,
