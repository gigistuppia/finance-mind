...
/**
 * Yahoo NO devuelve 404 para todos los símbolos inexistentes. Para muchos
 * devuelve HTTP 200 con un objeto basura, y ahí está el peligro:
 *
 ...
 *
 * Ese precio de diez mil millones tiene forma de número válido. Si entra al
 * pipeline, los indicadores lo procesan sin quejarse y el informe habla con
 * total seguridad de un activo que no existe.
 *
 * Un símbolo es válido solo si se cumplen las tres condiciones.
 */
const EXCHANGE_FANTASMA = 'YHD';

function metaEsValida(meta) {
  if (!meta) return { ok: false, motivo: 'sin metadatos' };
  if (meta.fullExchangeName === EXCHANGE_FANTASMA) {
    return { ok: false, motivo: `exchange "${EXCHANGE_FANTASMA}" (símbolo inexistente en Yahoo)` };
  }
  if (meta.currency == null) {
    return { ok: false, motivo: 'sin moneda' };
  }
  if (meta.regularMarketPrice == null) {
    return { ok: false, motivo: 'sin precio' };
  }
  return { ok: true };
}
...
async function fetchChart(symbol) {
  try {
    ...
    if (!metaEsValida(m).ok) return null;
...
async function getCandles(symbol, { range = '1y', interval = '1d' } = {}) {
  ...
  const validez = metaEsValida(result.meta);
  if (!validez.ok) throw new Error(`Símbolo inválido ${symbol}: ${validez.motivo}`);
  ...
  if (candles.length === 0) {
    throw new Error(`Sin velas utilizables para ${symbol}`);
  }

  // Los símbolos fantasma suelen traer datos congelados en 2019. Si el último
  // dato es viejo, el activo dejó de cotizar o el símbolo cambió — y analizar
  // velas de hace años como si fueran de hoy produce un informe seguro y falso.
  const diasDesdeUltima = (Date.now() - candles[candles.length - 1].t) / 86_400_000;
  if (diasDesdeUltima > 30) {
    throw new Error(
      `Datos rancios para ${symbol}: la última vela es de hace ${Math.round(diasDesdeUltima)} días`
    );
  }

  return {
    symbol: result.meta.symbol || symbol,
    // Nunca inventar una moneda: asumir USD sobre un papel argentino corrompe
    // todo el cálculo de P&L.
    //
    // Los índices devuelven currency "" porque legítimamente no tienen moneda
    // (verificado: ^MERV → ""). Se normaliza a null para que el consumidor
    // tenga que decidir explícitamente qué hacer, en vez de que una cadena
    // vacía pase por falsy y termine reemplazada por un "USD" inventado.
    // Un currency null de un símbolo fantasma ya fue rechazado por metaEsValida.
    currency: result.meta.currency || null,
    exchangeTimezoneName: result.meta.exchangeTimezoneName || '',
    diasDesdeUltimaVela: Math.round(diasDesdeUltima),
    candles,
  };
}

module.exports = { getQuotes, search, getCandles, fetchQuotesV7, fetchChart, metaEsValida };
